const localModel = {
  ...
  candidateId: { tag: 'select', required: true, pattern: '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}', fetchOption: 'api/v1/users/profile/USER' },
  advertisementId: { tag: 'select', required: true, pattern: '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}', fetchOption: 'api/v1/job_advertisements' }
};
...
document.addEventListener("DOMContentLoaded", async (evt) => {
  ...
  if (!entries || entries.length === 0) {
    table.innerHTML = '<tr><td>No applications available</td></tr>';
    return;
  }
  ...
  table.addEventListener('click', async (event) => {
    if (event.target.classList.contains('delete-button')) {
      ...
      try {
        const response = await fetch(`http://localhost:3000/api/v1/job_applications/${entryId}`, {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
          },
